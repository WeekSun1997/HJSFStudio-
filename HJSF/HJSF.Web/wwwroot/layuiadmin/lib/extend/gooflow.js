    ...
    function (exports) {
        ...
        var GooFlow = function (selector, property) {
            ...
            this.$workArea.on("click", { inthis: this }, function (e) {
                ...
                This.addNode(new Date().getTime().toString(), { name: "节点_" + This.$max, left: X, top: Y, type: This.$nowType });
                This.$max++;
            });
        ...
        GooFlow.prototype = {
            ...
            getItemInfo: function (id, type) {
                //console.info('getItemInfo');
                //console.info(id, type, this.$lineData[id]);
                switch (type) {
            ...
            render: function (options) {
                let that = this
                    ...
                    , flowConfig = {
                        ...
                        toolBtns: ["start round mix", "node", "end round"],
                        haveGroup: false,
                        useOperStack: true
                    };

                //合并参数
                let opt = $.extend({}, flowConfig, options.config || {});

                //初始化
                var flowPanel = new GooFlow($(options.elem), opt);
                //设置描述
                flowPanel.setNodeRemarks(remarks);
                //设置默认值
                if (options.data) {
                    flowPanel.loadData(options.data);
                }
                ...
                flowPanel.exportDataEx = function () {
                        ...
                        switch (_node.type) {
                            case "start round mix":
                                ...
                                if (fromlines[_node.id] == undefined) {
                                    layer.msg("开始节点无法流转到下一个节点", { icon: 2, time: 2000 });
                                    return -1;
                                }
                                break;
                            case "end round":
                                ...
                                if (tolines[_node.id] == undefined) {
                                    layer.msg("无法流转到结束节点", { icon: 2, time: 2000 });
                                    return -1;
                                }
                                break;
                            case "node":
                                if (_node.setInfo == null || !_node.setInfo.approvalUser || _node.setInfo.approvalUser.length <= 0) {
                                    layer.msg("请设置节点【" + _node.name + "】操作人员", { icon: 2, time: 2000 });
                                    return -1;
                                }
                                _flag = true;
                                break;
                            ...
                            case "join":
                                ...
                                break;
                            default:
                                layer.msg("节点数据异常！", { icon: 2, time: 2000 });
                                return -1;
                                break;
                        }
                    ...
                    if (startroundFlag == 0) {
                        layer.msg("必须有开始节点", { icon: 2, time: 2000 });
                        return -1;
                    }
                    if (startroundFlag > 1) {
                        layer.msg("只能有1个开始节点", { icon: 2, time: 2000 });
                        return -1;
                    }

                    if (endroundFlag == 0) {
                        layer.msg("必须有结束节点", { icon: 2, time: 2000 });
                        return -1;
                    }
                    if (endroundFlag > 1) {
                        layer.msg("只能有1个结束节点", { icon: 2, time: 2000 });
                        return -1;
                    }

                    if (fnodes.length != hnodes.length) {
                        layer.msg("分流节点必须等于合流节点", { icon: 2, time: 2000 });
                        return -1;
                    }
                    return data;
                }
                ...
                flowPanel.SetLineEx = function (id, data) {
                    flowPanel.setName(id, data.name, "line", data);
                }
                //节点/连接线双击事件
